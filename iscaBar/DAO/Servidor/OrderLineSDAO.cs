...
namespace iscaBar.DAO.Servidor
{
    public class OrderLineSDAO
    {
        ...
        public static async Task<String> UpdateAsync(OrderLine ordLine)
        {
            string URL = Constant.UrlApi + "bar_app/updateOrder";
            Uri URI = new Uri(URL);
            HttpClient client = new HttpClient();
            var js = JsonConvert.SerializeObject(ordLine);
        ...
        public static async Task<String> AddAsync(OrderLine ordLine)
        {
        ...
        public static async Task<String> DeleteAsync(OrderLine ordLine)
        {
            string URL = Constant.UrlApi + "bar_app/deleteOrder";
            Uri URI = new Uri(URL);
            HttpClient client = new HttpClient();
            var js = JsonConvert.SerializeObject(ordLine.Id);
            var httpContent = new StringContent(js, Encoding.UTF8, "application/json");
            Task<HttpResponseMessage> response = client.PutAsync(URI, httpContent);
            try
            {
